...
"what is this spot like?" — the whole archive condensed into one cube a
client slices by month, season, or time of day without ever refetching.
...
- `thresholdsMps` — the consumer's speed-band bounds the cube was binned
  with. A judgment parameter with no default, echoed so a reader knows
  which opinion shaped every `bandCounts` stack.
...
- `years` — the coverage ledger: per calendar year, `sampleCount` against
  ...
  that predate the station are trimmed; an interior silent year stays —
  that is a real outage, and hiding it would flatter the record.
...
upstream refuses fails the whole document — a silently missing year would
read as an outage. The yearly fetches cache by raw records, so one shared
...
- `climatologyCoverage(document)` — the honesty figure: samples held
  against the ledger's expectation.
