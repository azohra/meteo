...
> model's Fact table, with inline `[verified YYYY-MM-DD]` stamps beside
> individual measurements. Prior notes and community folklore are hypotheses,
> not sources.
...
Run the focused builder tests, then the full repository gates:

```sh
pnpm --dir forecast test   # builds briefing and grib, then the builder and registry tests
pnpm test                  # the full test suite
pnpm --dir site check      # astro check over the site, docs pages included
pnpm --dir site build      # the full site build
```

A passing run prints each suite's summary and exits 0; the registry test
fails when the catalogue and `registry.ts` disagree on entries or order. A
provider smoke run is an explicit network operation performed only after
fixture tests and review.
