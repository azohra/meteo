...
| --- | --- | --- |
| What will the launch do today? | HRDPS 1 km (an experimental feed) / 2.5 km, HRRR, RRFS (an experimental feed) or NAM nest 3 km | each other, then observations |
...
Both NAM entries carry a declared `sunset` in the catalogue (they retire 2026-10-06, and the
declared successor RRFS publishes through the same contract as an experimental feed), so weigh that
