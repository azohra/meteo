...
Eleven models publish through one contract, but their fields differ in measurement, cadence, and
availability. A blank can accurately state that a feed omits a quantity. The model catalogue
([`models.json`](https://github.com/azohra/meteo/blob/main/forecast/models.json)) declares what
each field means and whether each model publishes it.

ECCC and NOAA publish these model products openly and at no cost;
each centre documents its own conventions; the differences are real properties of different
modelling systems doing different jobs. The catalogue's job is to carry those properties to the
display intact.

<CapabilityMatrix />

The rest of this page is the stories behind those cells: where two identical-looking numbers are
different claims, and where a blank is itself the model's statement.
...
declares gust *semantics*, not gust presence (an hour-maximum, an instantaneous
sample, or nothing), and the readout wording follows the declaration: only the hour-maximum supports
...
smoke" is only half the fact. `"radiativelyCoupled"` (HRRR, RRFS) means the
...
separate [smoke document](/docs/briefing/smoke-document/) from another
...
arithmetic; see [Smoke and thermals](/logbook/smoke-and-thermals/).

## One model's lift is native, another's is converted

Every omega in the dataset is pascals per second with negative as lift, but not every model
computed one. Most capable models publish their own omega; RRFS publishes geometric vertical
velocity instead (metres per second), and the builder converts it at build time through the
hydrostatic relation ω ≈ −ρgw, with dry-air density from the level's own pressure and
temperature. The catalogue therefore declares vertical-velocity *provenance*, not presence:
`"omega"` for a provider's own pressure tendency, `"fromGeometricW"` for a converted value, so a
display can label converted lift differently from native lift instead of blending the two claims.
The conversion and its assumptions are recorded in the
[forecast model feed reference](/docs/forecast/forecast-model-feeds/).

## Surface-based CAPE

NOAA publishes a menu of CAPE variants (several parcel definitions, each answering
a different storm question), while ECCC publishes exactly one: surface-based. The forecast engine publishes the
...
given run; the [feed reference](/docs/forecast/forecast-model-feeds/) records the sentinel values
...
declined to answer; a frontend that fills it with 0 converts the gap into misinformation.
...
REPS publishes real ensemble soundings (every member's temperature, moisture, and
wind, reduced to percentiles under the accounting rules in
[Ensemble values](/docs/briefing/ensemble-values/)), yet none of the gust, CAPE,
...
altitude attached. HRRR, RRFS, and both NAM models publish low, middle, and high layer fractions
without a profile; GFS carries those layers as well as its profile; every
