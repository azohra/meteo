...
| Manifests, smoke, observation, run index | <code>{SCHEMA_VERSION}</code> | the same contract |
| Site catalogue and site context | 2 | `SITES_SCHEMA_VERSION`, `SITE_CONTEXT_SCHEMA_VERSION` |
...
- absence of an optional capability field means **not published**, never
  zero;
