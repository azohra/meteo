...
canadarasp — and cloud base was the last published quantity still computed by a rule of thumb.
...
oracle — the one thing I deliberately held fixed while everything around them moved. That debt is
recorded with gratitude in the [forecast derivations](/docs/forecast/derivation-science/); the estimate
...
stopped being an asset and became an approximation of a quantity the forecast engine could compute exactly
...
See every hour's shift below — the uniform cloud-base rise, and the two rust bars where the cap
came off.
