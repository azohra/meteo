...
import CapabilityProvenance from "../../components/logbook/CapabilityProvenance.astro";
import ModelLifecycleTimeline from "../../components/logbook/ModelLifecycleTimeline.astro";

# Adopting a model two months before its retirement

I put four candidate models through feed verification in one sitting: NOAA's NAM, both its 12 km
...
becomes operational. The obvious verdict is to skip a model with two months to live. I adopted it
anyway.

## The notice that reframed the evaluation

The NAM feeds themselves
[verified cleanly](/docs/forecast/forecast-model-feeds/): inventories, product split,
decode-level semantics. What changed the question was
NCEP's SCN 26-48: the Rapid Refresh Forecast System and its ensemble replace NAM, HREF, SREF, and
all HiresW domains but Guam, with implementation and retirement on the same day. There is no
overlap era: a builder written for NAM starts its life with a death date.

Skipping it would have been defensible: builders are real work. The reason to build anyway came
from verifying the successor first.
...
difference. Adopting NAM now buys the RRFS port started early, with two months of production
shakedown against a live feed and two months of NAM columns accumulating in the forecast history
before the switch.
...
3 km resolution and the parent's 84-hour horizon in one model.
...
entries from the day they were published (see [`models.json`](https://github.com/azohra/meteo/blob/main/forecast/models.json)). A
...
declaration that is supposed to mean "the model does not publish this" was quietly meaning "nobody
asked". The capability system exists to prevent exactly that, so I paid the debt in the same
change: both builders now fetch and publish omega.

The fix also changed the declaration's type. RRFS does not publish omega at all (its vertical
velocity is geometric w in m/s), so the port will convert during the forecast build (ω ≈ −ρgw, with density
...
grid-relative wind rotation, so it belongs in the forecast engine; `derive` stays a set of pure
...
I did not adopt RAP. It survives the October retirements and cycles hourly, but everything it
publishes for this product HRRR already publishes at finer resolution from the same buckets, and
its own successor is scheduled.

I did not adopt ICON global for now, the harder call of the two. It is the only candidate that
...
transport, and several contract fields it cannot fill. That entry stays open for later.

## Slug naming

Adding models forced me to answer what a slug is before minting more of them. The
...
One subtle clause: qualifiers belong only to
...
one the day a site crosses a cutout boundary.
