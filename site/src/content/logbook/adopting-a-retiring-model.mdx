...
asked". That is exactly the kind of dishonesty the capability system exists to prevent, so
I paid the debt in the same change: both builders now fetch and publish omega.
...
transport, and several contract fields it cannot fill. The entry it would occupy is a judgment
call deferred, not a door closed.
