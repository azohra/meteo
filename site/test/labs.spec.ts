...
test("the smoke lab's optical-depth slider moves the haze, the w* strip, and the readouts", async ({ page }) => {
  await page.goto("/logbook/smoke-and-thermals/", { waitUntil: "networkidle" });
  ...
  const thinOpacity = await mount.locator(".meteo-gram-smoke-cell").first().getAttribute("opacity");
  const thinStrip = await mount.locator(".meteo-gram-strip-thermalStrength").getAttribute("d");
  ...
  const thickOpacity = await mount.locator(".meteo-gram-smoke-cell").first().getAttribute("opacity");
  expect(thickOpacity).not.toBe(thinOpacity);
  // The adjusted w* strip actually derates as the plume thickens.
  const thickStrip = await mount.locator(".meteo-gram-strip-thermalStrength").getAttribute("d");
...
test("the smoke before/after figure reads at a glance: plume-free base, derated adjusted", async ({ page }) => {
  ...
  await expect(panels.nth(0).locator(".meteo-gram-strip-smoke")).toHaveCount(0);
  await expect(panels.nth(1).locator(".meteo-gram-strip-smoke")).toHaveCount(1);
  const baseStrip = await panels.nth(0).locator(".meteo-gram-strip-thermalStrength").getAttribute("d");
  const adjustedStrip = await panels.nth(1).locator(".meteo-gram-strip-thermalStrength").getAttribute("d");
...
test("the usable-lift lab's sink slider moves the line inside its everyday range", async ({ page }) => {
  await page.goto("/docs/forecast/derivation-science/", { waitUntil: "networkidle" });
...
test("the ensemble-spread lab redraws for each ensemble construction", async ({ page }) => {
  await page.goto("/docs/briefing/ensemble-values/", { waitUntil: "networkidle" });
