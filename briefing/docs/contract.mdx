...
Archived documents stay readable forever; only "published by a newer
writer" reads as invalid
...
The optional `site.timeZone` echo and its semantics (an old document
simply lacks the echo; that does not put the launch on UTC) are defined in
the
...
Switch on the value's shape, not a model slug; the
[profile reference](/docs/briefing/profile-document/#deterministic-and-ensemble-values)
...
- A stored profile's optional `semantics` and `site.timeZone` keep meaning
  with the document; neither's absence implies a default
  ([profile reference](/docs/briefing/profile-document/#run-site-and-semantics)).
