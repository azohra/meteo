...
runs, smoke, and observation pin `SCHEMA_VERSION` (<code>{SCHEMA_VERSION}</code>); the profile pins
its own exported `SITE_FORECAST_SCHEMA_VERSION` (<code>{SITE_FORECAST_SCHEMA_VERSION}</code>), and sites
and site context pin `SITES_SCHEMA_VERSION` (<code>{SITES_SCHEMA_VERSION}</code>) and
