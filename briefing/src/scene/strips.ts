...
export function buildStripSpecs(args: {
  ...
  /** The smoke strip's provenance statement. */
  ...
  if (overlays.pressure) {
    const values = hours.map((hour) => hour.surface.seaLevelPressureHpa / 10);
    stripSpecs.push({
      key: "pressure",
      label: "Pressure",
      unit: "kPa",
      ...
      bands: hours.map((hour) =>
        hour.bands.seaLevelPressureHpa
          ? {
              p25: hour.bands.seaLevelPressureHpa.p25 / 10,
              p75: hour.bands.seaLevelPressureHpa.p75 / 10,
            }
          : null,
      ),
          ...
          cells: row.values.map((value, index) =>
            value == null
              ...
              : {
                  ...
                  width: columnWidth,
                  className: "meteo-gram-cloud-cell",
  ...
  if (overlays.thermalStrength) {
    const wStar = hours.map((hour) => hour.derived.thermalVelocityMps);
    stripSpecs.push({
      key: "thermalStrength",
      label: "w*",
      unit: "m/s",
      values: wStar,
      bands: hours.map((hour) => hour.bands.thermalVelocityMps),
      minimum: 0,
      ...
      stripSpecs.push({
        ...
        cells: hours.map((hour, index) => {
          ...
          return {
            x: marginLeft + index * columnWidth,
            width: columnWidth,
            className: capped ? `${riskClass} meteo-gram-cape-capped` : riskClass,
          };
        }),
      });
    }
  ...
  if (overlays.buoyancyShear) {
    const perHour = hours.map((hour) => {
      const shear = surfaceToBoundaryLayerShearMps({
      ...
      const ratio = buoyancyShearRatio(hour.derived.thermalVelocityMps, shear);
      ...
      spec.cells = perHour.map((entry, index) =>
        entry.unopposed
          ? {
              x: marginLeft + index * columnWidth,
              width: columnWidth,
              className: "meteo-gram-bs-unopposed",
            }
          : null,
      );
    }
...
export function layoutStrips(
  ...
  args: {
    geometry: StripGeometry;
    /** Display-label overrides (MeteogramOptions.stripLabels); keys stay identity. */
  ...
  return stripSpecs.map((spec, stripIndex) => {
    ...
    const strip: MetricStrip = {
      ...
      key: spec.key,
      className: `meteo-gram-strip-${spec.key}`,
