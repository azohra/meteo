...
export const dsrObservationSchema = z.object({
  ...
  /** Measured downward shortwave flux at the surface, W/m²; instants without an accepted retrieval are absent — "not measured", never zero. */
  downwardShortwaveWm2: z
    .number()
    .describe(
      'Measured downward shortwave flux at the surface, W/m² (GOES-R ABI L2 DSR). Daytime product: instants without an accepted retrieval are absent from the series — "not measured", never zero.',
    ),
  /** The provider's nonzero DQF for this retrieval; absent means DQF 0 (good). DSR's DQF is binary, so 1 is its lumped "degraded/invalid" state — typically the sun beyond the 70° good-quality zenith bound: indicative, not quantitative. */
  quality: z
    .number()
    .int()
    .positive()
    .optional()
    .describe(
      "The provider's nonzero DQF for this retrieval; absent means DQF 0 (good). DSR's DQF is binary, so 1 is its lumped \"degraded/invalid\" state — typically the sun beyond the 70° good-quality zenith bound (the sunrise and sunset shoulders): indicative, not quantitative.",
    ),
});
...
export const aotObservationSchema = z.object({
  ...
  /** The provider's nonzero DQF for this retrieval; absent means DQF 0 (high). AOD's DQF is graded, so 1 is medium quality — the top-two set the smoke literature validated (Zhang, Kondragunta et al. 2020); strict quantitative use may filter to absent-only. */
  quality: z
    .number()
    .int()
    .positive()
...
export const sitesCatalogueSchema = z
  ...
  .describe(
    "sites.json — the hand-maintained site catalogue: every site the builders publish documents for. Identity and build selection only: humans author WHERE; the pipeline measures WHAT (site-context.json). The operator publishes it to the dataset root verbatim.",
  );
