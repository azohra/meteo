...
import { stabilityClass, STABILITY_CLASSES } from "../src/derive/stability.js";

describe("STABILITY_CLASSES", () => {
  it("keeps the eight stability classes in ascending boundary order", () => {
    expect(STABILITY_CLASSES.map((entry) => entry.className)).toEqual([
      "very-unstable",
      "unstable",
      "conditional-strong",
      "conditional",
    ...
    expect(STABILITY_CLASSES.map((entry) => entry.maxLapse)).toEqual([
      -3,
      -2.5,
      -2,
      -1.5,
