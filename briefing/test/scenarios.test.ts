...
function loadOutput(entry: ScenarioEntry, output: ScenarioOutput): LoadedOutput {
  ...
  expect(createHash("sha256").update(source).digest("hex"), `${label} sha256`).toBe(output.sha256);
  const rawProfile = JSON.parse(source) as unknown;

  const schemaResult = siteForecastSchema.safeParse(rawProfile);
  expect(
    schemaResult.success,
    `${label} must satisfy siteForecastSchema${
  ...
  const scene = buildMeteogramScene(profile!, { timeZone: entry.timeZone, launch });
  assertFiniteNumbers(scene, `${label} scene`);
  const svg = renderMeteogramSvg(scene, {
    idPrefix: `scenario-${entry.id}-${output.variant ?? "only"}`,
  });
...
function validateSvg(svg: string, label: string): void {
  ...
  expect(
    new Set(ids).size,
    `${label} SVG contains duplicate ids: ${duplicateValues(ids).join(", ")}`,
  ).toBe(ids.length);
...
function assertValidSvgPath(path: string, label: string): void {
  ...
  while (index < tokens.length) {
    ...
    expect(command, `${label} SVG path must begin each segment with a command`).toMatch(
      /^[A-Za-z]$/,
    );
    index += 1;
    ...
    while (index < tokens.length && !/^[A-Za-z]$/.test(tokens[index])) {
      expect(
        ...
        `${label} SVG path contains ${tokens[index]}`,
      ).toBe(true);
      count += 1;
      index += 1;
    }
    if (expected === 0) {
      ...
      expect(
        count,
        `${label} SVG path command ${command} has no coordinates`,
...
describe("generated teaching scenarios", () => {
  it("has unique scenario ids, output labels, paths, and comparison variants", () => {
    expect(
      ...
      `duplicate scenario ids: ${duplicateValues(scenarioIds).join(", ")}`,
    ).toBe(scenarioIds.length);
    const labels = outputs.map(({ entry, output }) => outputLabel(entry, output));
    expect(
      new Set(labels).size,
      `duplicate scenario outputs: ${duplicateValues(labels).join(", ")}`,
    ).toBe(labels.length);
    const paths = outputs.map(({ output }) => output.path);
    expect(
      new Set(paths).size,
      `duplicate scenario paths: ${duplicateValues(paths).join(", ")}`,
    ).toBe(paths.length);
    ...
    for (const entry of index.scenarios) {
      ...
      if (entry.kind === "comparison") {
        expect(
          ...
          `${entry.id} comparison needs multiple outputs`,
        ).toBeGreaterThan(1);
        expect(
        ...
        expect(
          entry.outputs[0].variant,
          `${entry.id} must not declare a comparison variant`,
        ).toBeUndefined();
      }
    }
  });
  ...
  it.each(outputs.map((loaded) => [outputLabel(loaded.entry, loaded.output), loaded] as const))(
    ...
    (_label, loaded) => {
      ...
      expect(loaded.svg).toContain(
        `id="scenario-${loaded.entry.id}-${loaded.output.variant ?? "only"}-cloud-hatch"`,
      );
    },
  );
  ...
  it("makes the convective cycle legible as a changing time-height field", () => {
    ...
    expect(firstSubfreezingHeights).toEqual([
      3150, 3150, 3150, 3150, 3650, 3650, 3650, 4250, 3650, 3150,
    ]);
    expect(new Set(firstSubfreezingHeights).size).toBe(3);
    expect(
      cycle!.scene.series.some((series) =>
        series.className.includes("meteo-gram-isotherm-freezing"),
      ),
    ).toBe(true);
...
describe("scenario SVG goldens", () => {
  ...
  it("matches the smoke-over-thermals ADJUSTED golden — the alternate view is a render option, not a different profile", async () => {
    ...
    const svg = renderMeteogramSvg(scene, { idPrefix: "scenario-smoke-over-thermals-adjusted" });
  ...
  it("the smoke-over-thermals correction is material, not merely nonzero", () => {
    ...
    const wPeak = (scene: ReturnType<typeof buildMeteogramScene>) =>
      Math.max(
        ...scene.strips
          .find((strip) => strip.key === "thermalStrength")!
          .values.map((value) => value ?? 0),
      );
    const base = wPeak(buildMeteogramScene(loaded!.profile, options));
    const adjusted = wPeak(
      buildMeteogramScene(loaded!.profile, { ...options, smokeAdjusted: true }),
    );
    expect(adjusted).toBeLessThan(base * 0.87);
    expect(adjusted).toBeGreaterThan(base * 0.5);
  });
});
